...
class ArucoPerceptionNode(Node):
    def __init__(self):
        ...
        # -----------------------------------
        self.REPROJECTION_ERROR_THRESHOLD = 0.5
        ...
        self.aruco_params.adaptiveThreshWinSizeStep = 1
        ...
        self.latest_depth_image = None  # We'll store the latest aligned depth here
        ...
        # -------------------------------------------
        # (a) Color camera info + image
        self.create_subscription(CameraInfo, self.CAMERA_INFO_TOPIC, self.camera_info_callback, 10)
        self.create_subscription(Image, self.IMAGE_TOPIC, self.image_callback, 10)

        # (b) Depth camera info + aligned depth image
    ...
    def depth_info_callback(self, msg):
        if self.depth_camera_matrix is None and self.depth_dist_coeffs is None:
            ...
            self.throttled_log("Depth intrinsics and resolution set (aligned).", "info")

    # -------------------------------------------
    # 10. Depth image callback (aligned depth)
    # -------------------------------------------
    ...
    def check_input_topics(self):
        if not self.camera_info_received:
            self.throttled_log("No color camera info received. Ensure /camera/camera/color/camera_info is publishing.", "warn")
        if not self.image_received:
            self.throttled_log("No color image received. Ensure /camera/camera/color/image_raw is publishing.", "warn")
        if not self.depth_info_received:
            self.throttled_log("No depth camera info received. Ensure /camera/camera/aligned_depth_to_color/camera_info is publishing.", "warn")
        if not self.depth_image_received:
            self.throttled_log("No depth image received. Ensure /camera/camera/aligned_depth_to_color/image_raw is publishing.", "warn")
        ...
        if ids is not None:
            for i, marker_id in enumerate(ids.flatten()):
                ...
                if marker_id == self.TARGET_MARKER_ID:
                    # NEW: Publish the target ID (e.g. 42) on its own topic
                    ...
                    # We still use the SolvePnP orientation
                    rmat, _ = cv2.Rodrigues(rvec)
                    transform_matrix = np.eye(4)
                    transform_matrix[:3, :3] = rmat
                    quat = quaternion_from_matrix(transform_matrix)

                    # Optionally draw 3D axes in the image (but set axis length to marker_size/2)
                    if self.VISUALIZE:
                        cv2.drawFrameAxes(
                            frame,
                            self.camera_matrix,
                            self.dist_coeffs,
                            rvec,
                            final_tvec,  # override with final tvec
                            marker_size / 2
                        )
